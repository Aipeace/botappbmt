...
@app.route("/app")
def mini_app():
    # Serve the Telegram mini-app UI from templates/index.html
...
async def run_bot_setup(render_url):
    ...
    if render_url:
        await bot.bot.set_webhook(url=f"{render_url}/webhook")
    else:
        logger.info("Skipping webhook setup because no RENDER_EXTERNAL_URL provided")
...
if __name__ == "__main__":
    ...
    if not render_url:
        logger.warning("RENDER_EXTERNAL_URL not set — running in local/Termux mode; webhook will not be set automatically")
    ...
    def _graceful_shutdown(signum, frame):
        ...
        try:
            if bot_app and bot_loop:
                try:
                    fut = asyncio.run_coroutine_threadsafe(bot_app.bot.delete_webhook(), bot_loop)
                    fut.result(timeout=10)
                    logger.info("Webhook removed")
                except Exception:
                    logger.exception("Error removing webhook")
        except Exception:
            logger.exception("Unexpected error during webhook removal")
        try:
            if bot_loop:
